...
static inline int ori_to_bin(_myfloat ori, int nbins)
{
    if (ori < 0)
        ori +=2 *M_PI;
...
static inline _myfloat bin_to_ori(_myfloat bin, int nbins)
{
    _myfloat ori = (bin+0.5)*2*M_PI/(_myfloat)nbins;
    if (ori > M_PI)
        ori -= 2*M_PI;
    return ori;
}
                                           ...
                                           _myfloat* hist)
{
    ...
    for(int si = siMin; si <= siMax; si++){
        for(int sj = sjMin; sj <= sjMax; sj++){
            ...
            //referential.
            _myfloat sX = (si-x_key)/sigma_key;
            _myfloat sY = (sj-y_key)/sigma_key;

            // gradient orientation (theta)
            _myfloat dx = imX[si*w+sj];
            _myfloat dy = imY[si*w+sj];
            _myfloat ori = modulus(atan2(dy, dx), 2*M_PI);

            // gradient magnitude with Gaussian weighing
            _myfloat r2 = sX*sX+sY*sY;
            _myfloat M = hypot(dx,dy) * exp(-r2/(2*lambda_ori*lambda_ori));
...
static _myfloat interpolate_peak(_myfloat h1, _myfloat h2, _myfloat h3)
{
    _myfloat offset = (h1-h3)/(2*(h1+h3-2*h2));
    return offset;
}


static void smooth_circular_histogram(int niter, _myfloat* hist, int nbins);
                                        ...
                                        _myfloat* principal_orientations)
{
    ...
    _myfloat max_value = array_max(hist, nbins);
    ...
    for(int i = 0; i < nbins; i++){
        ...
        if ( (hist[i] > threshold*max_value) && (hist[i]>hist[i_prev]) && (hist[i]>hist[i_next])){
            ...
            _myfloat offset = interpolate_peak(hist[i_prev], hist[i], hist[i_next]);
            // Add to vector of principal orientations (expressed in [0,2pi]
            principal_orientations[o] = bin_to_ori((_myfloat)i + offset, nbins);
            o++;
        }
    }
...
{
    _myfloat ori; // return value

    int i,i_prev,i_next;   // bin indices
    _myfloat offset;          // for normalization and interpolation

    // Smooth histogram : 6 iterated box filters
    smooth_circular_histogram(6, hist, nbins);

    // Find the histogram global extrema
    _myfloat t = find_array_max(hist, nbins, &i);
    (void)t;
    i_prev=(i-1+nbins)%nbins;
    i_next=(i+1)%nbins;
    // Quadratic interpolation of the position of each local maximum
    offset = interpolate_peak(hist[i_prev], hist[i], hist[i_next]);
    ori = bin_to_ori((_myfloat)i + offset, nbins);
    return ori;
}
 ...
 */
void sift_extract_feature_vector(_myfloat x_key, _myfloat y_key, _myfloat sigma_key,
                                 _myfloat theta_key,
                                 ...
                                 _myfloat* descr)
{
    ...
    _myfloat R =  (1+1/(_myfloat)Nhist)*lambda_descr*sigma_key;
    _myfloat Rp =  M_SQRT2*R;
                ...
                for(int i = MAX(0,i0);i<=MIN(i0+1,Nhist-1);i++){
                    for(int j = MAX(0,j0);j<=MIN(j0+1,Nhist-1);j++){ // looping through all surrounding histograms.
                        ...
                        descr[i*Nhist*Nbins+j*Nbins+k] += (1.-(gamma-floor(gamma)))
                                                         *(1.0-ABS((_myfloat)i-alpha))
                                                         *(1.0-ABS((_myfloat)j-beta))
                                                         *M;
...
{
    // Normalize
    _myfloat l2norm = array_l2norm(descr, n);
    // Threshold bins
    for(int i = 0; i < n; i++){

...
static void compute_keypoints_distance(_myfloat* dist,
                                       ...
                                       const struct sift_keypoints *k2)
{
    ...
    int n2 = k2->size;
    for(int i = 0; i < n1; i++){
        const _myfloat * v1 =  k1->list[i]->descr;
        for(int j = 0; j < n2; j++){
            const _myfloat * v2 =  k2->list[j]->descr;
            _myfloat d = euclidean_distance(v1, v2, dim);
            dist[i*n2+j] = d;
        }
    }
}
...
static void find_the_two_nearest_keys(const _myfloat* dist, int n1, int n2,
                                     int* indexA, int* indexB,
                                     _myfloat* distA, _myfloat* distB)
{
    for(int i = 0; i < n1; i++){
        int iA, iB;
        _myfloat dA, dB;
              ...
              int flag)
{
    ...
    _myfloat* dist  = (_myfloat*)xmalloc(n1*n2*sizeof(_myfloat));
    _myfloat* distA = (_myfloat*)xmalloc(n1*sizeof(_myfloat));
    _myfloat* distB = (_myfloat*)xmalloc(n1*sizeof(_myfloat));

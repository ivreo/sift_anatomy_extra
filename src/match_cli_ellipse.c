...
void print_usage()
{
    ...
    fprintf(stderr, "                                                                           \n");
    fprintf(stderr, "    -meth_flag    determines the value on which is applied the threshold   \n");
    fprintf(stderr, "               O :  threshold applied on euclidean distance                \n");
    fprintf(stderr, "               1 :  threshold applied on the ratio of euclidean distances  \n");
    fprintf(stderr, "    -thresh (250 / 0.6)  threshold value                                   \n");
        ...
        if (argv[i][0] == '-' && 0 == strcmp(argv[i]+1, opt))
        {
            ...
            if (i == argc-1){
                ...
                // if the option call is well formed
                    ...
                    strcpy(val, argv[i+1]); // TODO remplacer // copy string
...
int main(int argc, char **argv)
{
    ...
    sift_read_ellipses(k1, argv[1], n_hist, n_ori, n_bins, readflag);
    sift_read_ellipses(k2, argv[2], n_hist, n_ori, n_bins, readflag);

    // Matching
    matching(k1, k2, out_k1, out_k2A, out_k2B, thresh, meth_flag);

    // Print
    print_pairs_ellipses(out_k1, out_k2A);
    char name[FILENAME_MAX];
    if(verb_flag == 1){
        save_pairs_extra("OUTmatches.txt", out_k1, out_k2A, out_k2B);
        sprintf(name, "%s_im0.txt", label);
        sift_save_ellipses(out_k1, name, 1);
        sprintf(name, "%s_im1.txt", label);
        sift_save_ellipses(out_k2A, name, 1);
    }

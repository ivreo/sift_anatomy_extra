 ...
 *
 */
static struct octa *malloc_octa(_myfloat delta, int w, int h, int nSca, const _myfloat* sigmas){
    ...
    octave->h = h;
    octave->nSca   = nSca;
    octave->sigmas  = xmalloc(nSca*sizeof(_myfloat));       /* pointer to the array of level of simulated blurs */
    octave->imStack = xmalloc(nSca*w*h*sizeof(_myfloat));   /* pointer to the array of pixels  */
 ...
 *    sigmas[o][s] = deltas[o]*sigma_min*pow(2.0,(_myfloat)s/(_myfloat)nSca);
 *
 *
 */
                                                    ...
                                                    _myfloat sigma_min)   /* minimal scale in each octave (relatively to the sampling rate) */
{
    ...
    for(int o=0;o<nOct;o++){
        ...
        for(int s=0;s<nSca+3;s++){ /* nSca images + 3 auxiliary images*/
            sigmas[o][s] = deltas[o]/deltas[0]*sigma_min*pow(2.0,(_myfloat)s/(_myfloat)nSca);
        //    fprintf(stderr, "malloc scalespace delta %f sigma %f \n", deltas[o], sigmas[o][s]);
        }
    }
...
struct sift_scalespace* sift_malloc_scalespace_dog_from_scalespace(struct sift_scalespace* scalespace)
{
    ...
    for(int o=0;o<nOct;o++){
        ...
        dog->octaves[o]->sigmas  = xmalloc(nSca*sizeof(_myfloat));
        dog->octaves[o]->imStack = xmalloc(nSca*w*h*sizeof(_myfloat));

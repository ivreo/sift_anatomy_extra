 ...
 *
 */
static _myfloat* malloc_gaussian_symm_kernel(_myfloat sigma, int rad)
{
    assert(sigma>=0);
    _myfloat* gker = xmalloc((rad+1)*sizeof(_myfloat));
    gker[0] = 1.;
    if(sigma>0){
        _myfloat sum = gker[0];
        for(int i = 1; i <= rad; i++){
            gker[i] = exp(-0.5*(_myfloat)i*(_myfloat)i/sigma/sigma);
            sum += 2*gker[i];
        }
        for(int i = 0; i <= rad; i++)
            gker[i] /= sum;
...
static void convolve_symm(const _myfloat* in, _myfloat* out, int w, int h,
                          const _myfloat* xker, int r_xker,
                          const _myfloat* yker, int r_yker);


void sift_add_gaussian_blur(const _myfloat* in, _myfloat* out, int w, int h, _myfloat sigma){
    int r_gker = (int)ceil(4*sigma);  // ceil(4*sigma)
    _myfloat* gker = malloc_gaussian_symm_kernel(sigma, r_gker);
    convolve_symm(in,out,w,h,gker,r_gker,gker,r_gker);
    xfree(gker);
}

/** @brief sub sampling by factor 2, keeping sample (0,0) */
void sift_subsample_by2(const _myfloat* in, _myfloat* out, int wi, int hi){
    int wo = wi/2;
    int ho = hi/2;
    int i, j, i_p, j_p;
    for(i=0;i<ho;i++){
 ...
 * 
 */
void sift_oversample_bilin(const _myfloat* in , int wi,  int hi,
                          _myfloat* out, int wo, int ho,
                          _myfloat delta_out)
{
    assert(delta_out<=1);
    ...
    for(int i = 0; i < ho; i++){
        for(int j = 0; j < wo; j++){
            ...
            _myfloat x = i*delta_out;
            _myfloat y= j*delta_out;
            int im = (int)x;
            int jm = (int)y;
            int ip = im +1;
            int jp = jm +1;
...
//            fprintf(stderr, " (ip,im,jp,jm) = (%i,%i,%i,%i) \n", ip, im, jp, jm); 

            const _myfloat fractional_x = x - floor(x);
            const _myfloat fractional_y = y - floor(y);
...
static void convolve_symm(const _myfloat* in, _myfloat* out, int w, int h,
                          ...
                          const _myfloat* yker, int r_yker)
{
    _myfloat* im_tmp = xmalloc(w*h*sizeof(_myfloat));
    /* convolution along x coordinates */
    for(int i=0;i<h;i++){
        for(int j=0;j<w;j++){
            _myfloat sum = in[i*w+j] * xker[0] ;
    ...
    for(int i=0;i<h;i++){
        for(int j=0;j<w;j++){
            _myfloat sum = im_tmp[i*w+j] * xker[0];

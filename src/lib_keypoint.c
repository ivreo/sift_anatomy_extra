...
struct keypoint* sift_malloc_keypoint(int n_ori, int n_hist, int n_bins)
{
    ...
    key->n_hist = n_hist;
    key->n_bins = n_bins;
    key->descr = xmalloc(n_ori*n_hist*n_hist*sizeof(_myfloat));
    key->orihist = xmalloc(n_bins*sizeof(_myfloat));
    // set default value
    key->x = 0.;
    key->y = 0.;
    key->sigma = 0.;
...
void fprintf_one_keypoint(FILE* f, const struct keypoint* k, int n_descr, int n_bins, int flag)
{
    ...
    fprintf(f,"%f %f %f %f ", (double)k->x              // TEMP
                            , (double)k->y
                            , (double)k->sigma
                            , (double)k->theta);
    ...
    if (flag>1){
        ...
        for(int n = 0; n < n_bins; n++){
            fprintf(f,"%f ", (double)k->orihist[n]);
        }
    }
    // TMP
    // fprintf(f, "%f ", k->val);
                         ...
                         int flag)
{
    ...
    while(fgets(buffer, buffer_size, stream) != NULL){
        ...
        key->x = (_myfloat)x;
        key->y = (_myfloat)y;
        key->sigma = (_myfloat)sigma;
        key->theta = (_myfloat)theta;
        ...
        if (flag > 0){
            ...
            for(int i = 0; i < n_hist*n_hist*n_ori; i++){
                float dv;
                sscanf(buffer+pos, "%f %n", &dv, &read);
                key->descr[i] = (_myfloat)dv;
                pos +=read;
            }
        }
        if (flag > 1){

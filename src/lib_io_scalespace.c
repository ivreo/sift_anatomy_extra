...
void printImage_in_txtfile(const _myfloat *imageIn, int w, int h, const char *name)
{
    FILE* f = fopen(name, "w");
    for (int i = 0; i < w * h; i++) {
        fprintf(f, "%.50f ", imageIn[i]);
        if ( (i%30)==0){
            fprintf(f,"\n");
        }
    }
 ...
 *
 * */
void printImage(const _myfloat *imageIn, int w, int h, const char *name)
{
...
void nearestneighbor_interp(const _myfloat* in,int win, int hin, _myfloat* out, int wout, int hout)
{
    if ((hin != hout) || (win != wout)){
        _myfloat fh = (_myfloat)hin/(_myfloat)hout;
        _myfloat fw = (_myfloat)win/(_myfloat)wout;
        for(int i = 0; i < hout; i++){
            int k = floor( fh * (_myfloat)i );
            assert(k>-1); assert(k < hin);
            for(int j = 0; j < wout; j++){
                int l = floor( fw * (_myfloat)j );
...
void Msh2Lab(_myfloat M, _myfloat s, _myfloat h, _myfloat* L, _myfloat* a, _myfloat* b)
{
    *L = M*cos(s);
    *a = M*sin(s)*cos(h);
    *b = M*sin(s)*sin(h);
...
void Lab2xyz(_myfloat L, _myfloat a, _myfloat b, _myfloat* x, _myfloat* y, _myfloat* z)
{
    _myfloat vY = (L + 16.0)/116.0;
    _myfloat vX = a/500.0 + vY;
    _myfloat vZ = vY - b/200.0;
...
void xyz2rgb(_myfloat x, _myfloat y, _myfloat z, _myfloat *r, _myfloat* g, _myfloat* b)
{
    _myfloat var_x = x/100;  //x from 0 to  95.047  (observer = 2°, illuminant = d65)
    _myfloat var_y = y/100;  //y from 0 to 100.000
    _myfloat var_z = z/100;  //z from 0 to 108.883

    _myfloat var_r = var_x *  3.2406 + var_y * -1.5372 + var_z * -0.4986;
    _myfloat var_g = var_x * -0.9689 + var_y *  1.8758 + var_z *  0.0415;
    _myfloat var_b = var_x *  0.0557 + var_y * -0.2040 + var_z *  1.0570;
...
void print_sift_scalespace_rgb_nointerp(const struct sift_scalespace* scalespace, const char* basename)
{
 ...
 void coupe_oct(const char* name, const _myfloat* oct,  int h, int w, int ns, int x, int y)
{
    FILE* f = fopen(name, "w");
